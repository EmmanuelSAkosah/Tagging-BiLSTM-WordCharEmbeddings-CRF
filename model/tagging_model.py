...
class NERModel(BaseModel):
    """Specialized class of Model for tagging tasks such as NER and POS"""
    ...
    def add_cnn_word_embeddings_op(self):
        """
        ...
        with tf.variable_scope("chars"):
            if self.config.use_chars:
                ...
                _char_embeddings = tf.get_variable("_char_embeddings", shape=[self.config.nchars, self.config.dim_char],
                                                   dtype=tf.float32,
                                                   initializer=xavi())
                ...
                char_embeddings = tf.reshape(char_embeddings, shape= [-1, self.config.max_word_length, self.config.dim_char])
                ...
                for i, filter_size in enumerate(self.config.filter_size):
                    with tf.name_scope("conv-maxpool-%s" % filter_size):
                        ...
                        filter_shape = [filter_size, self.config.dim_char, 1, self.config.num_of_filters]
                        W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W_char")
                        b = tf.Variable(tf.constant(0.1, shape=[self.config.num_of_filters]), name="b_char")
                        ...
                        pooled = tf.nn.max_pool(
                            h,
                            ksize=[1, self.config.max_word_length - filter_size + 1, 1, 1],
                            strides=[1, 1, 1, 1],
                            padding='VALID',
                            name="pool")
                ...
                num_filters_total = self.config.num_of_filters * len(self.config.filter_size)
                # Concatinate all pooled features over 3th dimension
                self.h_pool = tf.concat(pooled_outputs,
                                        3)  # has shape= [Batch_size, 1, 1, len(FILTER_SIZE) * N_FILTERS]
                print("h_pool shape is: {}", self.h_pool.shape)
                print("word embedding shape is: {}", word_embeddings.shape)
                # Reshape data to shape= [Batch_Size, Sentence_Length, len(FILTER_SIZE) * N_FILTERS]
                self.h_pool_flat = tf.reshape(self.h_pool, shape=[-1, s[-2], num_filters_total])
                print("h_pool_flat shape is: {}", self.h_pool_flat.shape)
                #original self.h_pool_flat = tf.reshape(self.h_pool, [-1, s[1], num_filters_total])
                ...
                print("word embeddings after concat shape is: {}", word_embeddings.shape)
    ...
    def add_loss_op(self):
        ...
        with tf.name_scope("loss_summaries"):
            super().variable_summaries(self.loss)

    def build(self):
        # NER specific functions
        self.add_placeholders()
        if self.config.use_cnn_char_embedding:
            self.add_cnn_word_embeddings_op()
        else:
    ...
    def run_evaluate(self, test):
        ...
        f1100 = 100*f1
        acc100 = 100*acc
        # show acc and f1 on tensorboard
        with tf.name_scope("Accuracy"):
            self.variable_summaries(acc100)
        with tf.name_scope("F1_Score"):
            self.variable_summaries(f1100)

        return {"acc":acc100, "f1":f1100}
